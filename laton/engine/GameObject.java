...
public class GameObject {
	...
	public void aabbUpdate(AABB camAABB, ArrayList<AABB> mapAABB) {
			...
			if (camAABB.intersects(aabb)) {
				...
				if (!camAABB.intersects(aabb) || cam.inGround) {
					if (camAABB.max.y > aabb.min.y && camAABB.min.y < aabb.max.y && camAABB.min.y+4f > aabb.max.y) {
						cam.getPosition().y = aabb.max.y + (camAABB.max.y - camAABB.min.y);
		...
		if (cam.inGround && bounce > 0) {
			cam.inGround = false;
			cam.velocity.y += bounce;
			bounce /= 1.8f;
			if (bounce < 0.3f) {
				bounce = 0;
			}
	...
	public void update() {
		...
		if (sizeAABB != null && compareAABB != null) {
			aabbUpdate(new AABB(getPosition(), getPosition().add(sizeAABB)), compareAABB);
		}
	}
}